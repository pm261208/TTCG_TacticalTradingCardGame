...
public class ActionSystem : Singleton<ActionSystem> {
    ...
    private readonly Queue<(GameAction action, Action callback)> queue = new();
    ...
    public static void AttachPerformer<T>(Func<T, IEnumerator> performer) where T : GameAction {
        Type type = typeof(T);
        IEnumerator wrapedPerformer(GameAction action) => performer((T)action);
        if (performers.ContainsKey(type)) performers[type] = wrapedPerformer;
        else performers.Add(type, wrapedPerformer);
    }
